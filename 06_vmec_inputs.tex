...
\begin{itemize}
 ...
 \item \texttt{free\_boundary\_method} \\
       ...
       and \texttt{"only\_coils"} for using only the vacuum field due to external coils.
       The option \texttt{"only\_coils"} only works for zero pressure (\texttt{pres\_scale = 0})
       and zero net toroidal current (\texttt{curtor = 0}).
